...
local function rawlen(a)
   ...
   elseif type(a)=="table" then
      local n,pi=0,0
      for i in next,a do
         if type(i)=="number" and pi-i==1 then
            n=n+1
         else
            break
         end
   ...
   error("attempt to get length of a "..type(a).." value",3)
end
local function rawunm(a)
...
local function safe_call(func,...)
   if type(func)~="function" then
      error("attempt to call a "..type(func).." value",3)
   end
   return func(...)
end
--
	...
	while true do
		...
		if op < 18 then
				...
				elseif op > 3 then
					if op < 6 then
						if op > 4 then
							...
							memory[A + 1] = memory[B]
							memory[A] = rawget(memory[B],index)
						else
							--[[GETGLOBAL]]
					...
					elseif op > 6 then
						...
						memory[inst.A] = rawget(memory[inst.B],index)
					else
						--[[SUB]]
						local lhs, rhs
			...
			elseif op > 8 then
				if op < 13 then
					...
					elseif op > 10 then
						if op < 12 then
							...
							local ret_list = table.pack(safe_call(memory[A],table.unpack(memory, A + 1, A + params)))
				...
				elseif op > 13 then
					if op < 16 then
						if op > 14 then
							...
							return safe_call(memory[A],table.unpack(memory, A + 1, A + params))
						else
							--[[SETTABLE]]
							local index, value
							...
							rawset(memory[inst.A],index,value)
						end
					elseif op > 16 then
						--[[NEWTABLE]]
				...
				if rawequal(step,math.abs(step)) then
					loops = rawlowereq(index,limit)
				else
					loops = not rawlower(index,limit)
				end
		...
		elseif op > 18 then
			if op < 28 then
				if op < 23 then
					if op < 20 then
						--[[LEN]]
						memory[inst.A] = rawlen(memory[inst.B])
					elseif op > 20 then
						if op < 22 then
							--[[RETURN]]
							local A = inst.A
							...
							for i = B + 1, inst.C do str = rawconcat(str,memory[i]) end
				...
				elseif op > 23 then
					if op < 26 then
						if op > 24 then
							...
							if (rawequal(lhs,rhs)) == (inst.A ~= 0) then pc = pc + code[pc].sBx end
					...
					elseif op > 26 then
						...
						if (rawlower(lhs,rhs)) == (inst.A ~= 0) then pc = pc + code[pc].sBx end
			...
			elseif op > 28 then
				if op < 33 then
					if op < 30 then
						...
						if (rawlowereq(lhs,rhs)) == (inst.A ~= 0) then pc = pc + code[pc].sBx end
					...
					elseif op > 30 then
						...
						--[[UNM]]
						memory[inst.A] = rawunm(memory[inst.B])
					end
				elseif op > 33 then
					if op < 36 then
						if op > 34 then
				...
				local vals = {safe_call(memory[A],memory[A + 1], memory[A + 2])}
