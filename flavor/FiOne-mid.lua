...
local function on_lua_error(failed, err)
	...
	error(string.format('%s:%i: %s', src, line or -1, tostring(err)), 0)
end

local function run_lua_func(state, env, upvals, enable_middleware, middleware)
	...
	while true do
		...
		pc = pc + 1
		local goahead = true
		if enable_middleware and middleware~=nil then
			local big,info = false,{
				state=state;
				env=env;
				upvals=upvals;
				top_index=top_index;
			...
			for _,v in next, middleware do
				local res =  {v(info)}
				local to = res[1]
				if to == nil or to == "break" then
					goahead = false
					break
				elseif to == "stop" then
					big=true;
				...
				elseif to == "off" then
					...
					break
				elseif to == "set_pc" then
					pc = res[2]
					goahead = false
					break
				elseif to == "set_top_index" then
					top_index = res[2]
					goahead = false
					break
				elseif to == "set_op" then
					op = res[2]
					break
				elseif to == "close_and_return" then
					...
					if op < 33 then
						...
						elseif op > 30 then
							if op < 32 then
								...
								memory[inst.A] = lua_wrap_state(sub, env, uvlist, enable_middleware, middleware)
							else
								--[[TESTSET]]
								local A = inst.A
								local B = inst.B
...
function lua_wrap_state(proto, env, upval, enable_middleware, middleware)
	local function wrapped(...)
		...
		local result = table.pack(pcall(run_lua_func, state, env, upval, enable_middleware, middleware))
		...
		if result[1] then
			...
			on_lua_error(failed, tostring(result[2]))
